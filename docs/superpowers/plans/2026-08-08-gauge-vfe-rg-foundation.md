...
Assert true fresh-checkout click-to-run behavior in a sanitized subprocess with no editable install or `PYTHONPATH`, typed results, complete atomic numerical bundles, independent `collect_diagnostics` and `render_figures` toggles, and rendering entirely from saved metrics/arrays. `metrics.json` and core `arrays.npz` always exist; only `collect_diagnostics=True` adds `diagnostics.npz`. `render_figures=True` renders after numerical finalization into a sibling figure directory, while `False` creates none. Cover the full 2-by-2 toggle matrix for the finite path and both Gaussian toggle branches. A deliberately failing renderer must write a failed figure manifest without altering numerical status/metric bytes or replacing an existing valid image.

- [ ] **Step 7: Implement Gaussian launcher and pure figure replay**

All three root launchers add their adjacent `src/` directory before package imports and otherwise remain thin dictionary-driven files. `run_gaussian_lab.py` follows the same four-dictionary shape as the finite launcher. `make_figures.py` has an editable dictionary containing `run_dir`, `output_dir`, and requested figure names. Rendering reads only finalized saved artifacts and publishes each image through a temporary file plus atomic replace; an existing valid output is never clobbered by a failed replay. Render at least a signed VFE/Fisher identity-residual panel for the finite lab and matched generalized-spectrum panel for the Gaussian lab. Use a local `matplotlib.rc_context` so imports do not mutate global style: 3.5-inch publication width, 8/9/7-point body/axis/tick typography, hidden top/right spines, the Okabe-Ito colorblind-safe palette, and redundant marker/line encodings that remain legible in grayscale. Signed residuals use a symmetric scale with visible zero and tolerance boundaries; exact zeros are annotated rather than displaced. Export vector PDF plus 300-DPI PNG from the saved arrays/metrics with the noninteractive Agg backend. These are deterministic enumerated identity diagnostics, not sample estimates, so captions state `n=1 exact fixture` and do not invent error bars or significance marks.
