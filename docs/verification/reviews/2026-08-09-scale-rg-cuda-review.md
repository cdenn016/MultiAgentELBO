...
- Comparison orientation is `I_target C I_source^-1`; derivative factors are ordered as a nonautonomous cocycle.
...
- Full-subset interaction closure reconstructs the finite action; the pairwise-only mutation fails by the pinned exact amount.
...
- The standalone Python 3.12 worker validates the versioned JSON/NPZ envelope independently, including job, schema, array inventory, shape, dtype, per-array digest, file digest, environment-lock digest, request-response binding, and effective backend/dtype.
...
- CUDA code configures `CUBLAS_WORKSPACE_CONFIG=:4096:8` before process creation, deterministic algorithms before the first CUDA operation, and TF32 disabled. No CUDA code was executed in this session.
