...
The theorem is pointwise in one fixed (X). It makes no claim of cross-(X) sufficiency through (X_A), a smooth or Gaussian ambient family, a canonical presentation quotient, membership selection, patchwise gluing over (\mathcal U_A), a pair of parent sections, a full geometric meta-agent, canonical coarse-channel or partition selection, the downstream comparison theorem, autonomy, nonequilibrium, ontic action, a unique latent DAG or unique microscopic physics, or physical time.

It also makes no claim about canonical null-slice values (2026-08-16 remediation, finding L2). Every displayed quantity at the admitted (o) is a property of the *selected* posterior version. When the admitted (o) lies in a null set of the observation reference, a different admissible version changes the fine VFE while every frozen premise still holds; the package discloses this at `evidence/direct-derivation.md:45` and dispositions it as attack `A4`, but this limitation list previously omitted it.

Section 3 and Section 6 restate results already `ESTABLISHED` in this repository at the 8/15 diff base; `evidence/prior-results-map.md` gives the section-by-section correspondence and the Kullback--Leibler / Csiszár attribution (finding L1). `evidence/direct-derivation.md` itself is left byte-identical so that its recorded SHA-256 continues to bind the derivation the domain reviews read. `DYNAMICS-SCOPE` is a verified boundary/nonclaim, not a target ancestor, because the frozen target explicitly excludes dynamics from its static closure.
