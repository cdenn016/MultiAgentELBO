...
| CE-GLOBAL-QUOTIENT | a constant-rank pullback radical bundle automatically defines a global quotient manifold | \(U=\mathbb R^2\setminus\{0\}\), \(\rho(x,y)=y\), \(h=dy^2\); the horizontal null leaves give a line with doubled origin because the punctured \(y=0\) fiber has two connected components | \(h=\rho^*dy^2\) is already basic, but the connected-leaf space is non-Hausdorff; simple/Hausdorff quotient topology, not basicness of this \(h\), is the missing hypothesis |
...
| CE-PRESENTATION-RANK | a commuting diagram alone gives a smooth quotient-bundle isometry across a rank drop | \(F(t)=t^2\), \(J=\operatorname{id}\), \(\rho_A(t)=t^2\), \(\rho_B(s)=s\) | at zero \(Q_A=0\) and \(Q_B\cong\mathbb R\), while away from zero both are one-dimensional; diffeomorphism or surjective-submersion hypotheses and constant-rank strata are load-bearing |
...
| CE-BLOCK-BASIC | smooth pointwise direct and orthogonal blocks descend along retained-law fibers | \(\rho(x,y,z)=(x,y)\); \(B_1=\operatorname{span}(e_1,\partial_z)\), \(B_2=\operatorname{span}(e_2)\), with \(e_1,e_2\) rotating in \(z\) | \([\partial_z,e_1]=e_2\notin B_1+K\) and \([\partial_z,e_2]=-e_1\notin B_2+K\); the local distribution-projectability criterion fails |
