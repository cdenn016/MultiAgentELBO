...
at
`.verification/information-history/launcher-manual-final/artifacts/information_history/b782727d0d3f5a75360131bc1b0a77c60109974e6f8bf9762fbc100b72bd782e-20260809`.
...
the four coarse labels, evaluated at the fixture configuration map
`C(theta)`. Its target is the fixed-channel pushforward of the fine posterior.
...
| `semiconjugacy_defect_norm` | `0.5453674448304703` | `1e-6` detection threshold | pass | `OPEN` | `INCONCLUSIVE` | `PROJECT_NOVEL` |
...
history, defect norms ranged from `0.5406445310309916` to
`0.5453674448304703`.
...
raw coordinate length doubled while the covariantly transformed Fisher
duration residual was zero. The pinned wrong-plus-sign semiconjugacy mutation
missed its literal minus-sign oracle by `1.5`.
...
diagnostics; VFE gradients; quotient natural-gradient fields; path and chart
duration controls; and all typed semiconjugacy vectors needed to recompute the
five metrics.
...
`0.049218300000575255` seconds and reached `104265` bytes of peak Python memory
...
`ba4801f58e4470bcc676647297c0d55cc60b6597dffded3130230cf0d09a0d94`,
theory digest
...
| Fisher polygonal length is unchanged by the registered history relabeling and linear chart change. | `ESTABLISHED` | `EVIDENCE_VERIFIED` | `STANDARD` | Reparameterization and transformed-metric controls | Either information-duration residual exceeds tolerance after the same geometric path and transformed metric are confirmed. |
