...
| M-bind (amendment 11) | **C, negative on the ray — and it retracts the weak-coupling excuse.** Coupling-scale sweep: pair table scaled by `lambda ∈ {0, 1, 3, 10, 30, 100}` (pair sup `0` to `1.63`), all instruments re-run per grid point. The ceiling `I(X_B1;X_B2)` grows as `lambda^2` to within 2% across four decades (`4.97e-6` to `4.86e-2` nats at `k = 1`; `1.49e-5` to `1.36e-1` at `k = 3`), with saturation just beginning at `lambda = 100` — so by the top of the grid there is genuinely something to bind. The partition posterior does not care: the modal class is the **singleton partition at every lambda**, no crossing anywhere on the grid, and at `k = 1` the singleton mass *rises* with coupling (`0.586` to `0.605`); at `k = 3` it dips mildly at `lambda = 100` (`0.576` to `0.542`, direct rising to `0.336`) without approaching a crossing. Per the declared rule, no formation transition is licensed on this ray: the singleton preference is structural in the derived energy (the Ewens prior is `lambda`-independent, so the flow-averaged energy carries all movement), not a weak-coupling artifact. `R_MI` is flat-to-declining (`0.0230` to `0.0142` at `k = 1`; `0.0667` to `0.0641` at `k = 3`): transmission is **kernel-limited, not signal-limited** — the `t ≈ 0.15` per-edge factor is a property of the Bayes channel, and the channel does not adapt to signal. Sector gain stays small at all `lambda` (`+10%` to `+17%` at `k = 1`), pricing extended downward kernels down. Open contrast, recorded as hypothesis not conclusion: the M5 partition measurements on the fine falsification instance — where **observations** enter the energy — did find nontrivial persistent partitions, so the binding this theory's aggregation responds to may live in the likelihood sector, absent at coupling-only coarse levels: agents may condense around shared evidence, not mutual attraction. |
| M-anchor (amendment 12) | **C — environmental binding licensed.** The environment restored as a blockable agent class: one pinned agent per site (identity transport, two-channel divergence field, dressing rides the audited instruments; the coarse environment blocks by MAP under the declared downward kernels, and re-dressing the undressed coarse reproduces the audited coarse exactly). Per the declared rule, binding is licensed: **uniform** anchors flip the modal class to direct collapse (`0.552` at `k_env = 2`, `0.840` at `8`; realized anchored path `6 → 1`), while the **distinct** control anti-binds (singletons `0.902` at `k_env = 8`). **Shared-pairs** anchors move mass toward the anchor-aligned ratio-2 blocking eightfold (`0.025` to `0.196`) without crossing at the declared strengths — and the placement gap becomes physics (`0.19`): the posterior selects the placement matching the environment, not just the ratio. The hypothesis of the M-bind row is now supported at this seed: condensation follows shared evidence — uniform evidence binds everything, pairwise-shared evidence pulls toward the aligned pairing, no shared evidence anti-binds. |
| M-flow (amendment 12) | **C — the process reorders the landscape.** Joint Metropolis dynamics on `(x, R)` targeting the annealed joint `exp(-A) P(R) exp(-U)`, 3 replicas × 20,000 steps, seeds 0–2, acceptance `0.43`–`0.47`, validated against the exactly computed annealed marginal (agreement `0.01`–`0.02`). On the bare instance the annealed verdict **reverses the quenched one**: direct is modal (`0.441` exact, `0.461` sampled, unanimous across replicas) where amendment 10's quenched update gave singletons `0.586` — the Jensen gap between `E_w[U]` and the joint is qualitative. With shared-pair anchors at `k_env = 2` the annealed marginal returns to singletons modal (`0.489` exact) with direct suppressed. Combined reading with M-anchor: the M-part/M-bind singleton preference was a property of the quenched update on environment-free levels; the theory's process on levels with environment behaves differently in the declared directions, and both statements now sit in the record with their scopes attached. |
| M-cross-env (amendment 13) | **C — the declared plateau outcome.** Shared-pairs strength sweep `k_env ∈ {8, 16, 32, 64, 128}`, quenched and annealed objects computed exactly per point: the aligned placement's mass rises to `0.201` and **saturates** by `k_env = 32` (identical to four decimals thereafter, quenched = annealed as the flow pins — the frozen limit reached numerically), singletons stay modal at `0.617`; the distinct control is clean (`0.92` singletons, aligned `0.004`). No crossing: per the declared rule, pinning saturates before condensing, and the residual graph-group-plus-prior preference decides for singletons. The saturation exposes the mechanism: the per-site pinned dressing **factorizes over sites**, so at infinite inertia two agents pinned to the same value are indistinguishable from two agents with private duplicate evidence — a point-mass environment aligns but cannot correlate. Uniform-anchor direct condensation (M-anchor) worked through the alignment landscape, not correlation. Consequence, recorded as the declared next construction: binding-by-shared-evidence in the correlation sense requires **finite-inertia environmental agents**, whose fluctuations, integrated out, induce effective coupling between co-attached neighbors — the infinite-inertia (point-mass) limit implemented so far is exactly the regime in which shared and duplicated evidence coincide. |
| M-share (amendment 14) | **C — the strict licensing rule fails, and the channel decomposition it exposes is the result.** Finite-inertia shared environments implemented by exact marginalization (induced group potential decomposed by the anchored Moebius route; the m→∞ limit reproduces the amendment-13 pinned values to four decimals, and the private-duplicate control induces exactly zero pair coupling, both tested). Per the declared rule, correlation binding is **not licensed**: the aligned placement's mass rises monotonically in inertia toward the pinned plateau (`0.110 → 0.194` at `k_env = 8`) and never exceeds it. But the control contrast isolates the correlation channel as real and large: at `m = 0` the shared agents give the aligned pairing **9×** the private-duplicate control (`0.1105` vs `0.0125`) on an induced pair sup of `12.6`, and at `(k_env = 8, m = 0)` the free shared environment tips **direct** modal (`0.366`) — induced alignment-shaped attraction condenses the whole system, echoing the uniform-anchor mechanism. Reading: **alignment and correlation are competing channels for the aligned-placement statistic, not composing ones** — maximal alignment (the pinned limit) beats maximal correlation at this seed, and no pattern condenses the aligned pairing. The formation barrier that survives everything measured today is the derived energy's width penalty at the graph-group-plus-prior level. |
