...
| M-capacity (amendment 6) | **C, negative.** Sector-carrying parents (27 labels: presentation times belief-channel `Z_3` charge) do not raise one-step retention: `0.144` vs `0.156` at `k = 1`, `0.406` vs `0.441` at `k = 3`, with the constant-sector control reproducing the nine-state values exactly. Declared caveat: the sup statistic dilutes across alphabets; a capacity conclusion comparable across alphabets needs a mutual-information statistic, which is open. Capacity along this sector map was not binding at this seed. |
