...
Use the standard-Borel context, agent, and finite-design spaces of
\Cref{def:prob-structural-kernel-signatures}, let \(V\) be the finite agent
set, and let \(\mathfrak A\) be the finite interaction-record index set.
Agent \(i\) has the standard-Borel block space
\((\mathsf Y_{i,D},\mathscr Y_{i,D})\). Its state may contain a belief-law point in
$(\mathcal E_b)_{c_i}$, a model-law point in $(\mathcal E_m)_{c_i}$, and
declared auxiliary latent variables.  Write
$(\mathsf Y_D,\mathscr Y_D)=
\prod_{i\in V}(\mathsf Y_{i,D},\mathscr Y_{i,D})$. The normalized baseline is
the latent population kernel constructed in \Cref{cons:gen-finite-directed-law,hyp:gen-design-product},
\begin{equation}
\mathbb P_{\theta,V}^{Y}:
(\mathsf X,\mathscr X)\rightsquigarrow
(\mathsf Y_D,\mathscr Y_D).
\label{eq:obs-context-baseline-kernel}
\end{equation}
Each record \(a\) has a finite scope
$\partial a\subseteq V$, a standard Borel record space
$(\mathsf O_a,\mathscr O_a)$, and a normalized Markov kernel
\begin{equation}
K_a:(\mathsf X\times\mathsf Y_{\partial a,D},
\mathscr X\otimes\mathscr Y_{\partial a,D})
\rightsquigarrow(\mathsf O_a,\mathscr O_a).
\label{eq:obs-interaction-kernel}
\end{equation}
of the abstract type \eqref{eq:prob-record-kernel-signature}.
For the density presentation used in this chapter, one fixed probability or
sigma-finite measure $\nu_a$ dominates the whole family
$\{K_a(X,y_{\partial a,D},\cdot):(X,y_{\partial a,D})
\in\mathsf X\times\mathsf Y_{\partial a,D}\}$, and a finite jointly measurable
version
\begin{equation}
\ell_a:\mathsf X\times\mathsf Y_{\partial a,D}\times\mathsf O_a
\longrightarrow[0,+\infty),
\qquad
K_a(X,y_{\partial a,D},do_a)
=\ell_a(o_a\given X,y_{\partial a,D})\nu_a(do_a)
\label{eq:obs-interaction-density}
\end{equation}
is fixed once and for all.  Thus
$\int\ell_a(o_a\given X,y_{\partial a,D})\nu_a(do_a)=1$ for every
$(X,y_{\partial a,D})$.  The common $\nu_a$ and the displayed joint
measurability are family-level hypotheses, not recordwise choices.
The kernels and \(\mathbb P_{\theta,V}^{Y}\) are fixed by the generative parameters and structural
...
\(\mathbb P_{\theta,V}^{Y}\) have the corresponding locality. Bundle covariance is imposed by
...
the same joint law constructed by the normalized extension
\Cref{prop:gen-exact-normalization},
\begin{equation}
...
integrates to one, leaving the normalized latent law
\(\mathbb P_{\theta,V}^{Y}(\cdot\given X)\).
The factor scopes
...
$L_o,H_o,p_{\theta,V}(o\given X),
\boldsymbol\Pi_{\theta,V,o,X}$ and its block analogue is relative to this same fixed
context unless an $X$ argument is restored explicitly.

With $\nu=\bigotimes_{a\in\mathfrak A}\nu_a$, the complete record density is
the finite jointly measurable function
\begin{equation}
\ell(o\given y)=\prod_{a\in\mathfrak A}
\ell_a(o_a\given y_{\partial a}),
\qquad
\mathbb P_{\theta,V}(dy,do\given X)
=\mathbb P_{\theta,V}^{Y}(dy)\ell(o\given y)\nu(do).
\label{eq:obs-complete-record-kernel-density}
\end{equation}
For a fixed record $o$, define its likelihood before taking logarithms:
\begin{equation}
L_o(y):=\ell(o\given y)
=\prod_{a\in\mathfrak A}\ell_a(o_a\given y_{\partial a})
\in[0,+\infty).
...
\begin{equation}
H_o(y):=-\log L_o(y)
=\sum_{a\in\mathfrak A}-\log\ell_a(o_a\given y_{\partial a}),
\qquad -\log0:=+\infty.
...
\begin{equation}
0<p_{\theta,V}(o\given X)
=\int_{\mathsf Y_D}L_o(y)\mathbb P_{\theta,V}^{Y}(dy)
<+\infty,
...
=p_{\theta,V}(o\given X)^{-1}
L_o(y)\mathbb P_{\theta,V}^{Y}(dy)
...
For every probability measure \(Q\) on \(\mathsf Y_D\), define the collective
...
\(\mathcal L_o^{\rm ext}(Q)\leq\log p_{\theta,V}(o\given X)\) for every \(Q\),
with equality exactly when \(Q=\boldsymbol\Pi_{\theta,V,o,X}\) as full
measures. In particular, \(Q=\mathbb Q_{V,o,X}\) is the selected population
recognition instance. No domination or mean-field assumption on \(Q\) is
needed. \status{ESTABLISHED}

\paragraph{Proof.}
Apply the extended gap theorem \Cref{thm:elbo-extended-gap} with reference
\(\mathbb P_{\theta,V}^{Y}\) and likelihood \(L_o\). Nonnegativity and the equality condition are the
corresponding properties of relative entropy.  $\square$

If \(Q\ll\mathbb P_{\theta,V}^{Y}\) and the separate complexity and energy terms are defined without
an indeterminate $+\infty-\infty$, the same quantity has the familiar energy
presentation
\begin{equation}
\mathcal F_o^{\rm ext}(Q)
=\KL(Q\Vert\mathbb P_{\theta,V}^{Y})+\E_QH_o.
...
\section{The conditional VFE on an agent block}
\label{sec:local-conditional-vfe}

Let $B\subseteq V$ be nonempty and write $b=y_{B^c}$.  The same construction
therefore treats both the coordinates of a singleton agent $B=\{i\}$ and an
arbitrary nonempty aggregate block $B$ without changing probability models.
Choosing a block does not construct a meta-agent. That term is reserved for a
separately constructed coarse state, observation interface, evaluator,
recognition law, and update rule. Define
\begin{equation}
\mathfrak A_B=\{a\in\mathfrak A:\partial a\cap B\ne\varnothing\},
\qquad
g_{B,o}(y_B;b)=\prod_{a\in\mathfrak A_B}
\ell_a(o_a\given y_{\partial a}),
...
=\sum_{a\in\mathfrak A_B}-\log\ell_a(o_a\given y_{\partial a})
...
\(b\mapsto\mathbb P_{\theta,V,B}^{Y}(\cdot\given b)\) in the disintegration
\begin{equation}
\mathbb P_{\theta,V}^{Y}(dy_B,db)
=\mathbb P_{\theta,V,B^c}^{Y}(db)
\mathbb P_{\theta,V,B}^{Y}(dy_B\given b).
\label{eq:obs-baseline-block-disintegration}
\end{equation}
Every pointwise formula in $b$ below is relative to this declared version.
Two baseline-conditional versions agree
\(\mathbb P_{\theta,V,B^c}^{Y}\)-almost everywhere and
hence, by \eqref{eq:obs-posterior-outside},
\(\boldsymbol\Pi_{\theta,V,o,X,B^c}\)-almost everywhere.
...
\begin{equation}
p_{\theta,V}(o\given X)
=\int w_B(b)\mathbb P_{\theta,V,B^c}^{Y}(db),
\qquad
\boldsymbol\Pi_{\theta,V,o,X,B^c}(db)
=p_{\theta,V}(o\given X)^{-1}
w_B(b)\mathbb P_{\theta,V,B^c}^{Y}(db).
...
\end{equation}
Then \(\boldsymbol\Pi_{\theta,V,o,X,B^c}(\mathsf R_{B,o})=1\). Indeed, finite collective evidence
makes \(w_B<+\infty\) outside a \(\mathbb P_{\theta,V,B^c}^{Y}\)-null set, and the posterior outside
...
be \(\mathbb P_{\theta,V,B^c}^{Y}\)-full. \status{ESTABLISHED}

For $b\in\mathsf R_{B,o}$ relative to the fixed baseline-conditional version,
the posterior conditional is
\begin{equation}
\boldsymbol\Pi_{\theta,V,o,X,B}(dy_B\given b)
=Z_B(b)^{-1}g_{B,o}(y_B;b)
\mathbb P_{\theta,V,B}^{Y}(dy_B\given b),
...
version for which
\begin{equation}
\boldsymbol\Pi_{\theta,V,o,X}(dy_B,db)
=\boldsymbol\Pi_{\theta,V,o,X,B^c}(db)
\boldsymbol\Pi_{\theta,V,o,X,B}(dy_B\given b).
\label{eq:obs-posterior-block-disintegration}
\end{equation}
The posterior joint determines this kernel only
\(\boldsymbol\Pi_{\theta,V,o,X,B^c}\)-almost everywhere. In particular, neither the conditional nor
...
\(Q_{B^c}\ll\boldsymbol\Pi_{\theta,V,o,X,B^c}\) and are therefore independent of these choices.
\status{ESTABLISHED}
...
+\KL\bigl(
r_B(\cdot\given b)
\Vert\boldsymbol\Pi_{\theta,V,o,X,B}(\cdot\given b)
\bigr).}
\label{eq:obs-local-vfe}
\end{equation}
Thus $\mathcal L_{B,o}^{\rm ext}(r_B;b)\leq\log Z_B(b)$, with equality
exactly at \(r_B=\boldsymbol\Pi_{\theta,V,o,X,B}\). The singleton $B=\{i\}$ is one agent's local VFE;
an arbitrary nonempty $B$ is the corresponding block-coordinate aggregate VFE.
No domination or mean-field condition on $r_B$ is needed.
\status{ESTABLISHED}

\paragraph{Proof.}
Apply \Cref{thm:elbo-extended-gap} conditionally with reference
\(\mathbb P_{\theta,V,B}^{Y}(\cdot\given b)\) and likelihood \(g_{B,o}(\cdot;b)\). The equality
...
=\KL\bigl(
r_B(\cdot\given b)
\Vert\mathbb P_{\theta,V,B}^{Y}(\cdot\given b)
\bigr)
...
\(Q_{B^c}\ll\boldsymbol\Pi_{\theta,V,o,X,B^c}\), then, in \([0,+\infty]\),
\begin{equation}
\KL(Q\Vert\boldsymbol\Pi_{\theta,V,o,X})
=\KL(Q_{B^c}\Vert\boldsymbol\Pi_{\theta,V,o,X,B^c})
+\E_{Q_{B^c}}
\KL\bigl(r_B(\cdot\given Y_{B^c})
\Vert\boldsymbol\Pi_{\theta,V,o,X,B}
(\cdot\given Y_{B^c})\bigr).
\label{eq:obs-posterior-block-kl-chain}
\end{equation}
The baseline chain rule, whenever its regular conditional versions are used,
likewise reads
\begin{equation}
\KL(Q\Vert\mathbb P_{\theta,V}^{Y})
=\KL(Q_{B^c}\Vert\mathbb P_{\theta,V,B^c}^{Y})
+\E_{Q_{B^c}}
\KL\bigl(r_B(\cdot\given Y_{B^c})
\Vert\mathbb P_{\theta,V,B}^{Y}
(\cdot\given Y_{B^c})\bigr).
...
\(Q_{B^c}\ll\boldsymbol\Pi_{\theta,V,o,X,B^c}\). This fixed outside marginal condition is load bearing. Suppose
\(\KL(Q\Vert\boldsymbol\Pi_{\theta,V,o,X})<+\infty\) and
\(\KL(Q'\Vert\boldsymbol\Pi_{\theta,V,o,X})<+\infty\); the chain rule then makes both conditional-KL
...
terms agree. The common finite constant
\(-\log p_{\theta,V}(o\given X)\) cancels, as does the
...
When \(\mathbb P_{\theta,V}^{Y}=\bigotimes_i\rho_i\), define the total
correlation of a theorem-local joint recognition law \(Q\) by
\begin{equation}
...
+\E_Q\left[\sum_{a\in\mathfrak A}E_{a,o}\right],
...
=\sum_{a\in\mathfrak A}|\partial a|E_{a,o}(y_{\partial a}).
...
\(D_{ij}:\mathsf Y_D\to\mathbb R\) is finite and measurable. At the selected
...
\end{equation}
where \(L_o^Y:\mathsf Y_D\to[0,+\infty)\) is a finite measurable function
