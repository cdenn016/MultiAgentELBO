...
The picture is this. Begin with local probabilistic agents: each has beliefs, a law over model presentations, an evaluator that turns a sampled presentation into a normalized generative mechanism, and an access interface specifying which records can inform its recognition law. Couple those local objects through normalized interaction-record kernels and gauge-covariant comparisons. The resulting fixed joint supports an exact variational principle relating log evidence, a variational bound, and a relative entropy. This is not an identification of variational free energy with a physical action principle. The renormalization question is then which of this structure survives when microscopic degrees of freedom are replaced by effective ones, and how much information is destroyed in the replacement.
...
Begin with a finite population \(V\). Each agent \(i\) carries local belief and model-law sections \(q_i^{b;o,X}\) and \(q_i^{m;o,X}\), an access map \(\operatorname{Acc}_i\), an agent recognition law \(\mathbb Q_{i,o,X}\), and the generative-model datum \((q_i^m,\operatorname{ev}_i)\). A sampled presentation \(m_i\) is evaluated incidence-wise as a normalized kernel \(K^{X,a}_{i,m_i}=\operatorname{ev}_{i,a}(m_i)\), and normalized record kernels \(K_a\) attach the declared interaction records. The contextual manifold \(\mathcal C\), principal \(G\)-bundle \(\mathscr P_G\to\mathcal C\), and associated belief and model bundles make these local descriptions comparable across frames; transport around a closed loop may have nontrivial holonomy. On a finite design, the normalized local mechanisms and record kernels construct \(\mathbb P_{\theta,V}\), while a measurable selector chooses \(\mathbb Q_{V,o,X}\in\operatorname{Cpl}(\mathbb Q_{\bullet,o,X})\). Neither population law is an agent, mind, or observer. With those joints fixed, the exact identity
\begin{equation}
  \log p_\theta(o\mid X)
  =\mathcal L(\Rec_X;X)
...
\caption{A translation between the manuscript's objects and standard physics vocabulary. The right column is an orientation aid; each row is qualified in the section that treats it.}\label{tab:dictionary}\\
\toprule
...
\(q_i^{b;o,X},q_i^{m;o,X},\operatorname{Acc}_i\) & An agent's belief and model-law response sections together with its declared information interface. These are local data, not marginals extracted from a pre-existing population observer. \\
\(q_i^m,m_i,\operatorname{ev}_{i,a}(m_i)=K^{X,a}_{i,m_i}\) & Respectively a law over model presentations, one sampled presentation, and the normalized generative mechanism that the presentation denotes at an incidence. The three types must not be identified. \\
\(\mathbb P_{\theta,V}\) & The normalized latent--record population law constructed from the local agent mechanisms and interaction-record kernels. It is not primitive population knowledge. \\
\(\operatorname{Cpl}(\mathbb Q_{\bullet,o,X})\), \(\mathsf S_{o,X}\), \(\mathbb Q_{V,o,X}\) & The class of joints with the declared agent marginals, a hypothesized measurable selector, and its selected correlated joint. Existence of a coupling is established; a particular selector is additional structure. \\
...
A law fiber has no canonical vector-space structure. The ambient set of probability measures is convex, but arbitrary linear combinations need not remain positive or normalized, and a restricted model family need not be closed under mixtures. There is therefore no covariant derivative in the usual vector-bundle sense on a merely measurable law fiber, and no curvature two-form acting on it by matrix multiplication. Differential statements are made only on a declared smooth tier, where the law fibers are finite-dimensional smooth parametrized-measure models closed under the group action and the action is smooth. Even there, the Fisher tensor is a Riemannian metric only when it is nondegenerate, and it is called a semimetric until nondegeneracy is proved.

A law fiber is also not a kernel fiber, and using law fibers does not remove an agent's generative model. The agent's model datum is \((q_i^m,\operatorname{ev}_i)\): \(q_i^m\) represents its law over model presentations, a presentation \(m_i\) is a sample from the appropriate incidence space, and \(\operatorname{ev}_{i,a}(m_i)=K^{X,a}_{i,m_i}\) is the normalized Markov kernel that presentation denotes. These are three different types. The evaluator is not assumed injective, so distinct presentations may denote the same mechanism, and no regularity is claimed for the quotient.
...
On a finite design, the normalized local agent mechanisms \(G_{\theta,i,D}\) compose along a topological order and the normalized interaction-record kernels \(K_a\) attach each record once. This constructs the population joint
\begin{equation}
  \mathbb P_{\theta,V}:(\mathsf X,\mathscr X)\longrightarrow\Law(\mathsf Y_D\times\mathsf O_D),
  \label{eq:generative-signature}
\end{equation}
from local data rather than postulating it as a primitive population object. Because each factor is separately normalized, the composition normalizes exactly and there is no partition function to compute. Each access interface supplies an agent law \(\mathbb Q_{i,o,X}\). Their product proves that the coupling class \(\operatorname{Cpl}(\mathbb Q_{\bullet,o,X})\) is nonempty, but the marginals generally do not determine a joint. The correlated recognition law \(\mathbb Q_{V,o,X}\) is therefore chosen by a declared measurable selector from that class; it may correlate agents, belief and model coordinates, and design points, and no factorization is imposed. The two population laws are complete compatibility objects used for evidence and collective accounting, not a super-agent that possesses them.
...
\begin{equation}
  \ell(o\mid y)=\prod_{a\in\mathfrak A}\ell_a(o_a\mid y_{\partial a}),
  \qquad
  H_o(y)=\sum_{a\in\mathfrak A}-\log\ell_a(o_a\mid y_{\partial a}),
...
\end{equation}
and for a nonempty block \(B\), conditioning on the outside state \(b=y_{B^c}\) gives a block free energy \(\mathcal F_{B,o}(r_B;b)\) of exactly the same form, built from the records incident to \(B\). The singleton \(B=\{i\}\) is one agent, while an arbitrary larger \(B\) is only a block-coordinate aggregate. Choosing a block does not construct a meta-agent. That term is reserved for a separately constructed coarse state, observation interface, evaluated generative mechanism, recognition law, and update rule.
...
The bounded Neo-Kantian reading begins before any hierarchy. Local frames, model evaluators, and access interfaces are constitutive conditions under which a record becomes inferentially meaningful for an agent, while remaining revisable across learning or theory change. The fixed base, bundle, finite design, and measurable spaces are analyst-declared coordination scaffolds that let these local interfaces be compared; the architecture is local-first, not background-free. The constructed \(\mathbb P_{\theta,V}\) and selected \(\mathbb Q_{V,o,X}\) encode population compatibility across the local interfaces, but neither is a population mind, universal observer, or additional agent.

Participation means reciprocal inferential dependence mediated by relational records inside one fixed normalized model. It does not entail consciousness, physical creation by observation, ontological closure, or an identification of variational free energy with a physical action principle. Those are separate philosophical or physical claims, not consequences of the probability construction.
...
Everything above describes the \emph{passive} channel, in which the coarse coupling is inherited through blocking with attention frozen, and the follow-up measurements of 2026-08-17/18 changed the headline. Neither structural rescue lifts the passive channel above one: thickening the block boundary raises the one-step pair retention only sublinearly ($0.156$, $0.441$, $0.564$ at one, three, and six cut couplings per boundary) and saturates below one, and enlarging the parent alphabet with the smallest sector label, the block's belief-channel $Z_3$ charge in the singlet/triplet sense of Section~\ref{sec:trees-loops}, root-framed through the spanning-tree transport, raises retention ($0.209$ against $0.156$ at one cut coupling, $0.568$ against $0.441$ at three, with the constant-sector control exact) while remaining below one; an earlier null ($0.144$) came from a family-referenced charge that fails gauge covariance and was retired by the 2026-08-18 audit. What restores interaction is the theory's own re-binding rule: regenerating each level's attention from the flow-averaged transported divergence over the conserved connection and folding the alignment energy into the coarse action. Under that declared rule, itself gauge covariant to $10^{-11}$, the per-ratio composites converge to \emph{interacting} fixed structures: pairwise sup $0.579$ at ratio two and $0.625$ at ratio three, each about a quarter above its own one-step injection, with the maps still contracting. The physical moral is sharp: in this theory a tall hierarchy keeps its horizontal glue not by inheriting it but by actively re-establishing it at every level, using exactly the two ingredients blocking conserves, the connection and the coarse beliefs. The typed-cocycle verdict is unchanged by regeneration: composition defects stay order one ($0.17$--$0.27$) and the interacting fixed structures remain ratio-typed ($0.64$ relative sup), so which aggregation grain a hierarchy uses remains physically meaningful all the way to its endpoints.
...
What survives is the anchored star of Equation~\eqref{eq:precision-addition}, and with it the participatory content that motivated the fold in the first place. At a variational fixed point the apex belief is a transported pooling of every constituent's current belief, and each constituent's effective prior in its own update is that apex belief transported back down. Each agent's input is constituted by the apex update and the apex input is constituted by the agents' updates. The local evaluators and record kernels still define a fixed constructed joint. This is a loop in an inference algorithm, not a cycle in the factorization of the model: every step is a coordinate update of a trial law against a joint that remains fixed, acyclic, and normalized, so nothing reads a posterior into a generative kernel.
...
{\small
\begin{longtable}{>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}p{0.80\textwidth}}
\caption{The boundary of the theory, compressed. Each row is a summary of many individually tagged statements; the manuscript's own ledger appendix is authoritative.}\label{tab:status}\\
\toprule
...
\addlinespace
Not claimed &
That \(\mathcal C\) is physical spacetime, or carries a Lorentzian signature, causal cones, a measure, or a field equation; that Fisher duration is physical time, proper time, thermal time, or a quantum clock; that RG depth is inference time; that graph-link holonomy evidences base curvature or bundle topology; that \(\mathbb P_{\theta,V}\) or \(\mathbb Q_{V,o,X}\) is a mind, super-agent, or universal observer; that participation entails consciousness, physical creation by observation, background independence, or a physical action principle; that autonomous agency emerges; and that any structural-realist reading follows from the invariance results. \\
\end{longtable}
}

\begin{provedbox}
The load-bearing achievements are a local-first typed gauge-covariant kinematics for probabilistic agents, a normalized population joint constructed from local mechanisms and records, a nonempty coupling class together with an explicit selector obligation for correlated recognition, one exact variational identity, an exact potential identity making local multi-agent updates coordinate descent on one collective free energy, a canonical source-selection ensemble with a correctly normalized row free energy, an exact accounting of the information destroyed by any normalized coarse channel, a full static pointwise parent probabilistic datum, and a set of sharp finite counterexamples that fence off invalid shortcuts.
\end{provedbox}
...
The first is that the architecture is local-first and the gauge structure is not decorative. Agents holding probabilistic beliefs and model laws in local frames genuinely have a comparison problem, transport genuinely resolves it, and loop transport genuinely fails to be trivial. A model-law fiber does not erase an agent's generative model: the law describes its uncertainty over presentations, and the evaluator turns a presentation into a normalized mechanism. The lattice analogy is exact at the level of the link variables and their Wilson loops, and the trivializing criterion is literally the pure-gauge criterion. Where the analogy stops is that probability-law fibers are convex rather than canonically linear, so the group acts by pushforward. The constructed and selected population joints appear downstream to encode compatibility and dependence; they are not a population subject.
...
The manuscript's own fast path for a physicist is worth following. Begin with the local agent sections, access maps, presentation evaluators, agent mechanisms, and interaction-record kernels in the introduction, geometry, and probability chapters. Then follow the normalized construction of \(\mathbb P_{\theta,V}\), the coupling class and selector for \(\mathbb Q_{V,o,X}\), and the evidence identities in the generative and ELBO chapters. On a second pass read the local--collective potential theorem and the attention construction, then the covariant pullback and relational-history chapters. On a third pass take the three-operation distinction, the Markov coarse theorem, the exact network closure, and the typed scale diagram. The Gaussian spine is then the interaction cone, the recognition costs and Fisher geometry, and finally Gaussian aggregation, Kron closure, holonomy, fixed rays, and the numerical gates. The obstruction and interpretation chapters separate the exact negative results from the empirical and interpretive commitments, and the claim ledger collects every open obligation in one place.
