...
block progress. The short version is that an exact finite theory of coarse-graining
exists and is in good order, while a renormalization \emph{group} does not exist:
there is no rescaling map, no beta function, no blocking ratio, and no
relevant--irrelevant classification for the two-channel gauge network. Recent work
has sharpened three things worth knowing. Descent of the multiscale free energy, on
its own, ranks no partition at all. The directed edge-event law supplies a genuine
gauge-invariant length scale where the attention weights do not. And nontrivial
holonomy on a cyclic graph can make a coherent meta-agent not merely expensive but
nonexistent. Everything involving the words flow, persistence, or emergence remains
unproved.
\end{abstract}
...
works and is not in doubt: normalized coarse-graining channels, the free-energy
identity they satisfy, the construction of a parent from a block of children, the
correct way to coarse-grain directed attention, and the exact treatment of non-flat
connection data. The selection layer does not work, and a recent result explains
why in a stronger way than had been appreciated: the free energy is exactly
indifferent to the choice of partition unless extra structure is declared. The
dynamical layer, meaning persistence of blocks, closure across scales, and any
identification of the descent parameter with physical time, remains open, and the
project's own claim ledger has said so consistently.
...
\label{eq:vfe}
\end{equation}
where $\bm\Pi$ is the posterior. The evidence term is the ground-state energy of the
...
tower carries no partition function at all: the only normalizer anywhere is the
evidence. Local normalization of node and edge potentials does not give a finite
global normalizer, as the Gaussian pair with edge potential $e^{c y_1 y_2}$,
$c \ge 1$, shows by having a negative exchange eigenvalue. Any same-time reciprocal
...
restriction of the recognition law to constant rows, the row free energy
\begin{equation}
\Phi^b_i = \sum_j \beta_{ij} \E_{\mathbb Q} D^b_{ij}
+ \tau^b_i \KL(\beta_i \Vert \pi^b_i)
\end{equation}
is an exact sector of the collective free energy, with softmax optimum
...
is degenerate. Note also that the temperature is not free. Taking $\tau \ne 1$ gives
the same optimizer but a different objective value, and it is an evidence bound only
for a correspondingly tempered generative law.
...
directions are exact. What does not exist is the rescaling and identification map
...
symmetrization used to define a directed graph Laplacian. On the resulting symmetric
operator, effective resistance and commute time \emph{are} metrics, and the diffusion
time at which the entropic susceptibility peaks supplies an intrinsic scale that
requires no embedding. Running the two channels separately gives two scale spectra,
and their disagreement is itself a diagnostic: a system whose belief and model
channels peak at different times has no single hierarchy.
...
\subsection{Holonomy can forbid a meta-agent outright}
...
block-formation energy has a strictly positive floor set by the geometry alone. A
...
bounded trajectories converge to critical points. Multistability, hysteresis, and long
...
Statement & Status \\
\midrule
Exact free-energy identity and its conditional decomposition & established \\
Coarse-graining chain rule with nonnegative discarded information & established \\
Parent construction from a block by one normalized channel & established \\
Coarse attention by pushing the edge-event law and disintegrating & established \\
Exact closure generates hyperedges; pairwise closure is false & established \\
Holonomy retained as a distribution of dressed transports & established \\
Free energy alone ranks no partition & established \\
Gauge-invariant Laplacian, effective resistance as a metric & established \\
Linear aggregation is the unique choice compatible with the chain rule & established \\
Empty holonomy fixed set forbids a coherent parent & established \\
Mean-field block-formation instability threshold & conditional \\
Maximum-weight blocking on the resistance scale & conjecture \\
Rescaling map, semigroup, beta function, relevance classes & open \\
...
hypotheses not verified for the actual system; ``conjecture'' means favored and
unproved.}
\end{table}
...
score and the support of the dressed-transport distribution; measure partition residence
times against the belief relaxation time and fit exit times against noise amplitude; and
...
The exact finite results are solid and several are certified in the program's own
released derivation packages. The most recent package covering the multiscale
construction released with terminal status \textsc{inconclusive}, and no claim in it
carries a verified state, because no independent model checked the derivations. An
external review of that package found four substantive defects and six lesser ones,
all now repaired; one of the four had been raised by the package's own adversarial pass
and then wrongly dismissed on a repair that was described but never implemented. That
episode is the reason for the reporting discipline in this note: treat the exact finite
layer as reliable, and treat anything containing the words flow, persistence, or
emergence as unproved until a specific obligation in the table above is discharged.
