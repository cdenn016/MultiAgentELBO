...
measured in \S10.1: in the coarse-graining direction it is one to two percent of the
pairwise coupling.
...
and an earlier version of this section reported the second while describing the
first.
...
\begin{tabular}{@{}lccc@{}}
\toprule
partition & two-body & three-body & ratio \\
\midrule
$\{1,2\}\{3,4\}\{5,6\}$ & $9.55\times10^{-2}$ & $1.79\times10^{-3}$ & $0.019$ \\
$\{2,3\}\{1,6\}\{4,5\}$ & $1.12\times10^{-1}$ & $7.02\times10^{-4}$ & $0.006$ \\
$\{1,2,3\}\{4,5\}\{6\}$ & $1.07\times10^{-1}$ & $1.93\times10^{-3}$ & $0.018$ \\
\bottomrule
\end{tabular}
\end{center}

so a three-body parent coupling is generated, it is nonzero, and it is between half
a percent and two percent of the pairwise coupling. The orders decay rather than
staying comparable. That is the ordinary situation in real-space renormalization,
...
the affirmative. Blocking the two cycles into two parents leaves only two coarse
variables, so no three-body term can arise at all, which is what a reading of the
architecture as two meta-agents joined pairwise correctly suggests.
...
structure. There the generated three-body term among the children is
$6.7\times10^{-2}$, comparable to the pairwise terms rather than one percent of
them, and the interaction magnitudes do not decay with order at all: on a
five-agent block the one- through five-body sup norms are $0.93$, $1.61$, $0.85$,
$1.62$ and $0.64$. That is a genuine failure of any order truncation, but it is a
statement about describing children without naming their parent, not about the
coarse theory.
...
three-body coupling one to two percent of its pairwise one.}
\label{fig:star}
\end{figure}
...
parent coupling at one or two percent of the pairwise scale, with the orders
decaying. A pairwise coarse theory is therefore a truncation carrying a residual of
