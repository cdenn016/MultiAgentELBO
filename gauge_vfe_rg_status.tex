...
How large that residual is depends on which variables are eliminated, and is
measured in \S10.1: in the coarse-graining direction the generated three-body
coupling is between a tenth of a percent and one and a half percent of the pairwise
one.
...
though not for the reason first reported: the closure verdict recorded here earlier
has been withdrawn, and in the renormalization direction the coarse theory is
dominantly pairwise, as \S10.1 sets out. There is no timescale separation between
partition persistence and belief relaxation. And the designed partition into the two cycles is never selected,
...
\subsection{Which direction the closure question is about}

Two eliminations are possible in a two-scale model and they answer different
questions, so the closure statement has to name one. Integrating out the children
and keeping the parents is the renormalization direction, the one that asks what
theory the coarse variables obey. Integrating out the parent and keeping the
children is the opposite move, zooming out and then back in, and it is not a step
of any renormalization scheme. The two give quantitatively different answers here,
and an earlier version of this section carried the second under a heading that
named the first. The numbers below are also post-audit: the coarse measurement was
found on 2026-08-17 to broadcast three of the eight edge divergence tables
transposed, to omit the $\kappa$ couplings the rest of the model applies to the same
edge law, to weight parents uniformly where the declared law is not uniform, and to
normalize its blocking kernel over children rather than parents, so that it did not
preserve the partition function. All four are corrected here, and the coarse action
is checked against the declared per-edge energy to $3\times10^{-15}$.

In the renormalization direction the coarse theory is dominantly pairwise. Blocking
the six agents into three parents and eliminating the children exactly gives a
parent action whose interaction content is
...
\midrule
$\{1,2\}\{3,4\}\{5,6\}$ & $9.90\times10^{-2}$ & $6.20\times10^{-4}$ & $0.0063$ & $0.00016$ \\
$\{2,3\}\{1,6\}\{4,5\}$ & $1.12\times10^{-1}$ & $1.54\times10^{-4}$ & $0.0014$ & $0.00004$ \\
$\{1,2,3\}\{4,5\}\{6\}$ & $1.35\times10^{-1}$ & $1.98\times10^{-3}$ & $0.0147$ & $0.00051$ \\
\bottomrule
\end{tabular}
\end{center}

so a three-body parent coupling is generated, it is nonzero, and it is between a
tenth of a percent and one and a half percent of the pairwise coupling, with a
flow-weighted residual of a few parts in ten thousand. Only the first partition is
a declared candidate of the design; the other two were added afterwards and are
exploratory. Of the six declared candidates only two carry the three blocks a
three-body coupling needs, and the second of those has no connected block, so the
first row is the only declared partition the measurement can reach. That is the
ordinary situation in real-space renormalization,
where decimating an Ising model generates four-spin couplings and nobody regards
the model as thereby broken; what matters is whether the generated couplings are
small and whether they are irrelevant, and the first of those is answered here in
the affirmative. The single above-pair order does decay away from the pair order,
$1.47$ then $9.9\times10^{-2}$ then $6.2\times10^{-4}$, but the instance caps the
parent pool at three labels, so a three-block partition admits exactly one order
above pair order and a hierarchy of decay is not something this instance can
exhibit. Blocking the two cycles into two parents leaves only two coarse variables,
so no three-body term is expressible there at all; that is a structural fact about
the arithmetic and not evidence for any reading of the architecture.

The opposite elimination behaves quite differently, and it is worth showing because
it is the generic mechanism by which a hidden common cause induces higher-order
structure. Put on the same dimensionless footing as the table above, the
three-to-two ratio there is $0.28$ and $0.21$ on the two declared blocks, against
$0.0063$ in the coarse direction, a factor of roughly forty. The sharper contrast
is that the magnitudes do not decay with order at all: on a five-agent block the
one- through five-body sup norms are $1.21$, $2.19$, $1.25$, $2.24$ and $1.41$.
That is a genuine failure of any order truncation, but it is a statement about
describing children without naming their parent, not about the coarse theory.
...
the children without naming the parent. This is the elimination that runs
\emph{against} the renormalization direction, shown because it is the generic
mechanism by which a hidden common cause induces higher-order structure; the
coarse-graining direction, which eliminates children and keeps parents, generates a
three-body coupling of order a percent of its pairwise one.}
\label{fig:star}
\end{figure}

The mechanism is the one shown in Figure~\ref{fig:star}. The downward kernel
factorizes over the children of a block given its parent,
\begin{equation}
K_\downarrow\big(z_4,z_5,z_6\,\big\vert\,z_1\big)
= K(z_4\mid z_1)\,K(z_5\mid z_1)\,K(z_6\mid z_1),
\label{eq:star-factorized}
\end{equation}
so conditionally on $z_1$ the children are independent and the block is a star,
which is pairwise and carries no loop. Marginalizing the parent leaves the
effective action
\begin{equation}
H_{\rm eff}(z_4,z_5,z_6)
= -\log\sum_{z_1}P(z_1)\,\prod_{i\in I}K(z_i\mid z_1),
\label{eq:star-marginal}
\end{equation}
and the logarithm of a sum of products is not additively separable in the
children. A common cause, once integrated out, couples everything it touched
simultaneously. The interaction content is read off by Moebius inversion against a
declared ground configuration $z^\circ$,
\begin{equation}
\Phi_A(z_A)=\sum_{B\subseteq A}(-1)^{|A|-|B|}\,
H_{\rm eff}\big(z_B,z^\circ_{A\setminus B}\big),
\label{eq:mobius}
\end{equation}
and a coarse theory is pairwise exactly when $\Phi_A$ vanishes for every $A$ with
$|A|\ge3$.

The Ising star is the same computation in closed form. Take one center spin
$s_0$ in a field $h_0$ coupled to three leaves,
$H=-h_0s_0-\sum_{k}J_ks_0s_k$, and eliminate the center exactly:
\begin{equation}
\sum_{s_0=\pm1}e^{h_0s_0+xs_0}=2\cosh(h_0+x),
\qquad x=\sum_{k}J_ks_k,
\end{equation}
so the effective leaf action is $-\log 2-\log\cosh(h_0+x)$. Only the cubic term of
$x$ carries $s_1s_2s_3$, with coefficient $6J_1J_2J_3$, and
$\tfrac{d^3}{dx^3}\log\cosh(h_0+x)\big\vert_{0}=-2\,\mathrm{sech}^2(h_0)\tanh(h_0)$,
so the generated three-leaf coupling is
\begin{equation}
\Phi_{\{1,2,3\}}
=2\,\mathrm{sech}^2(h_0)\,\tanh(h_0)\,J_1J_2J_3
\label{eq:ising-star}
\end{equation}
at leading order. It vanishes exactly when the field vanishes and when any single
coupling does, and both degeneracies are reproduced. The exact coefficient of the
finite model converges to \eqref{eq:ising-star} as the couplings shrink, with
successive ratios $0.470$, $0.815$, $0.951$, $0.988$ and $0.997$ over halvings of
the coupling scale, the gap quartering each time as an order-$\varepsilon^2$
correction should.

So the honest closure statement is a quantitative one rather than a categorical
one. Exact blocking does generate couplings outside the pairwise family, which was
never in doubt and is what \eqref{eq:ising-star} illustrates. In the direction that
a renormalization step actually runs, what it generates is small: a three-body
parent coupling between a tenth of a percent and one and a half percent of the
pairwise scale, $0.0063$ on the one declared partition the measurement can reach. A pairwise coarse theory is therefore a truncation carrying a residual of
that size, not a misdescription. That is a far weaker obstruction to a second scale
than the earlier reading suggested, and it leaves the missing rescaling map as the
binding constraint rather than one obstruction among two.

What this does not settle is whether the generated coupling is \emph{irrelevant} in
the technical sense, which is the question that decides whether truncation is safe
under iteration rather than merely at one step. Smallness at one step and
irrelevance under many are different claims, and only the first is measured here.
The second needs a fixed point to linearize about.
