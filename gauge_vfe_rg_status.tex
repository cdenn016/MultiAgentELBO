...
way. Write $\Vert\Phi_k\Vert_\infty=\max_{|A|=k}\max_{z_A}|\Phi_A(z_A)|$ for the
unweighted size of the $k$-body content. On a five-agent block,
\begin{equation}
\big(\Vert\Phi_1\Vert_\infty,\dots,\Vert\Phi_5\Vert_\infty\big)
=(0.93,\;1.61,\;0.85,\;1.62,\;0.64),
\end{equation}
which does not decay with order at all, so there is no small parameter organizing
a hierarchy of the kind an effective field theory relies on. Against the Boltzmann
measure the model itself assigns, $\rho\propto e^{-H_{\rm eff}}$, the same content
measured as
\begin{equation}
\frac{\mathbb E_\rho\big\vert\sum_{|A|\ge3}\Phi_A\big\vert}
{\mathbb E_\rho\big\vert H^{\rm pair}_{\rm eff}-\mathbb E_\rho H^{\rm pair}_{\rm eff}\big\vert}
\approx 0.02 .
\end{equation}
The higher-order terms are large in magnitude and concentrated where the measure
is small. Whether truncation is controlled therefore depends on the norm, and in
the norm that governs expectations it appears to be.
